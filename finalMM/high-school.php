...
<body>
	<div id="wrapper" class="wrapper">
		...
		<main>
            ...
            echo "<h4>Here is a list of users that have confirmed colleges they will be attending.</h4>";
            echo "<h4>Reach out to them via email to try to make friends, hear advice, or just chat!</h4>";
            echo "<br>";

            $query = "select firstName, lastName, email, subject, collegeName from weavers.Users where collegeName IS NOT NULL and year = 'HS - SR' order by subject";

            echo "<br>";
            echo "<h1>High school seniors</h1>";

            if ($stmt = $con->prepare($query)) {
                $stmt->execute();
                $stmt->bind_result($firstName, $lastName, $email, $subject, $collegeName);
                echo "<table>";
                echo "<tr>";
                echo "<th>First name</th>";
                echo "<th>Last name</th>";
                echo "<th>Email</th>";
                echo "<th>Future major</th>";
                echo "<th>College</th>";
                echo "</td>";
            ...
            echo "<hr>";
            echo "<br>";
            echo "<h1>High school juniors</h1>";

			$query = "select firstName, lastName, email, subject, collegeName from weavers.Users where collegeName IS NOT NULL and year = 'HS - JR' order by subject";
